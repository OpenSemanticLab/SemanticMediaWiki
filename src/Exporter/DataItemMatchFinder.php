...
class DataItemMatchFinder {
	...
	public function matchExpElement( ExpElement $expElement ) {
		...
		if ($dataItem && $dataItem instanceof DIWikiPage) {
	...
	private function matchToUnknownWikiNamespaceUri( $uri ) {
		 ...
		 * ToDo: Use swivt:page (after removing / spliting at $wgArticlePath + namespace, 
		 * e. g. Category-3A) or rdfs:isDefinedBy or rdfs:isDefinedBy (after removing / 
		 * spliting at Special:ExportRDF/ + namespace, e. g. Category-3A)
